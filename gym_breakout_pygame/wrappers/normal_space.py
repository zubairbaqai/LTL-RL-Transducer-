...
class BreakoutNMultiDiscrete(BreakoutSkipper):
    ...
    def __init__(self, config: Optional[BreakoutConfiguration] = None):
        ...
        BricksNumbers = self._bricks_matrix_space.n
        for i in range(BricksNumbers[0]):
        ...
        self.observation_space = MultiDiscrete(Tuples)
    ...
    def observe(cls, state: BreakoutState):
        ...
        for i in range(state.brick_grid.bricksgrid.shape[0]):
            for j in range(state.brick_grid.bricksgrid.shape[1]):
                obs.append(state.brick_grid.bricksgrid[i][j])
